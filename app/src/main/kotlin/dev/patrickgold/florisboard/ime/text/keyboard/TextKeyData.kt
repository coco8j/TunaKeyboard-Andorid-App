 ...
 */
/**
 * 이 데이터 클래스는 단일 키와 그 속성을 설명합니다.
 *
 * @property type 키의 타입을 나타냅니다. 일부 동작은 키 코드(`code`)와 타입(`type`)이 모두 일치해야
 *  성공적으로 실행될 수 있습니다. 기본값은 `KeyType.CHARACTER`입니다.
 * @property code UTF-8로 인코딩된 문자 코드로, 시스템에 전달되는 실제 데이터를 나타냅니다.
 *  기본값은 0입니다.
 * @property label UI에 표시되는 문자열입니다. 시스템에 전달되는 데이터에는 사용되지 않으며,
 *  일반적으로 코드와 동일해야 합니다. 기본값은 빈 문자열입니다.
 */
