 ...
 */
/**
 * `locale.displayName()`을 반복적으로 호출하는 것을 방지하기 위해 캐시를 사용하여 언어 이름을 계산합니다.
 * 이 메서드는 `LocaleNative.getLanguageName()` 메서드를 호출하며,
 * 이는 상당히 느린 ICU 데이터 테이블을 사용하여 언어 이름을 조회합니다.
 * 이 캐시는 마지막으로 계산된 표시 이름만 저장하지만,
 * 서브타입이 변경될 때 한 번 재계산하는 것은 문제가 되지 않으며,
 * 같은 언어에 대해 반복적으로 계산하는 것이 성능에 더 큰 영향을 미칩니다.
 */
